...
for ele in data.iterrows():
    ...
    try:
        ...
        article_sentiment = pd.DataFrame(sentiment).value_counts().index[0][0]
        irony_sentiment = pd.DataFrame(group_xi).value_counts().index[0][0]
        ...
        # Write to Database
        final = pd.DataFrame(result, columns=['tweet_id', 'username', 'raw_content', 'view_count', 'sentiment', 'date_created'])
